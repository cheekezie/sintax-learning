...
const MetricCard = ({ icon: Icon, title, value, change, changeType, period }: MetricCardProps) => {
...
const OverviewMetrics = ({ totalReceive, totalStudents, totalFees }: OverviewMetricsProps) => {
  const metrics = [
    ...
    },
    {
      ...
      value:
        totalStudents && totalStudents > 0 ? formatCurrency((totalReceive ?? 0) / totalStudents) : formatCurrency(0),
      change: '0%',
      changeType: 'neutral' as const,
      period: 'Per student',
    },
    ...
    <div className='bg-bg/60 backdrop-blur-xl rounded-2xl p-6 shadow-sm font-sora'>
      <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 items-stretch gap-y-4 gap-x-0'>
        {metrics.map((metric, index) => {
          const leftBorderForMd = index % 2 !== 0 ? 'md:border-l md:border-slate-200/60' : '';
          const leftBorderForXl = index % 4 !== 0 ? 'xl:border-l xl:border-slate-200/60' : '';
          return (
            <div key={index} className={`${leftBorderForMd} ${leftBorderForXl} pl-0 h-full`}>
              <MetricCard {...metric} />
            </div>
          );
        })}
